...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += "بوت"
    await msg.reply(f"**» استخـراج الجلسـة **{ty}** بواسطـه‌𝘼𝙍𝘼𝘽𝙀𝙍 جـارٍ..**.")
    user_id = msg.chat.id
    ...
    if not is_bot:
        t = "**⎆ يـرجـى إرسـال رقـم هاتفـك مـع رمـز الدولة مثــال 📱: +96400000000**"
    else:
    ...
    text = f"**هذا هو {ty} كـود جلسـة** \n\n`{string_session}` \n\n**مستخـرج مـن :** @U_9_DBOT\n🍒 **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلالـه قنـاة السورس @Haider_1h1 ."
    try:
        if not is_bot:
